...
global WEB_APP_URL := "https://script.google.com/macros/s/AKfycbyaCVhyGYYf_dxsrBqioyT625whyunINgRqPmRclnp5rCvHQibLUNo5TfnHPRl3Mim6/exec"
...
Gosub, VerifyOwnership
Return

VerifyOwnership:
    global VERIFIED_KEY, WEB_APP_URL, settingsFile
    ...
    fullURL := WEB_APP_URL . "?username=" . URLEncode(savedUser) . "&computer=" . URLEncode(compName)
    ...
    if (status != 200) {
        MsgBox, 16, HTTP Error, Failed to contact server. Status: %status%
        ExitApp
    }
    if InStr(resp, """error""") {
        MsgBox, 16, Error, % "Server error:`n" resp
        ExitApp
    }

    owns := true  ; <-- force override for testing

    ; if this is the very first registration, server will return exists:false
    if InStr(resp, """exists"":false") {
        MsgBox, 64, Registered, Successfully registered %savedUser% on this PC!
    }
...
CombinedVerify:
{
    InputBox, username, Verify Premium Macro, Enter your Roblox username:
    if ErrorLevel
        ExitApp

    EnvGet, compName, ComputerName
    fullURL := WEB_APP_URL . "?username=" . URLEncode(username) . "&computer=" . URLEncode(compName)
    Http := ComObjCreate("WinHttp.WinHttpRequest.5.1")
    Http.Open("GET", fullURL, false)
    Http.Send()
    status := Http.Status
    ...
    owns    := InStr(resp, """owns"":false") ? false : true
    exists  := InStr(resp, """exists"":true")
    matched := InStr(resp, """matched"":true")

    if (!owns) {
        MsgBox, 48, No GamePass, You do NOT own the required GamePass. Access denied.
        ExitApp
    }
    if (exists && !matched) {
        MsgBox, 48, Sheet Check, Username %username% is already registered on another PC.
        ExitApp
    }
    if (!exists) {
        MsgBox, 64, Registered, Successfully registered %username% on this PC!
    }
